...
def procesar_ventas(lista_ventas):
    # Variables estrictamente locales (nacen y mueren en esta función)
    total = 0
    conteo = 0
    positivos = 0
    mejor = -1
...
def comparar_periodos(ventas_actual, ventas_anterior):
    # Reutilizamos procesar_ventas para no escribir la lógica dos veces
    ...
    cambio_pct = 0.0
    if total_anterior > 0:
        # Calculamos porcentaje y lo redondeamos a 1 decimal
...
def analisis_completo(datos_salas):
    # Definimos pequeñas funciones auxiliares (helpers) de forma local
    # Esto asegura que nuestro pipeline funcione de forma totalmente independiente
    ...
    def extraer_nombre(tupla): 
        return tupla[0]
        
    def extraer_total(tupla): 
        return tupla[1]

    # 1 y 2. Calcular los totales y crear el resumen
    resumen = []
    for nombre, ventas in datos_salas.items():
        # Usamos nuestra herramienta maestra "reducir"
        total_sala = reducir(sumar_valores, ventas, 0)
        resumen.append((nombre, total_sala))
        
    # 3. Filtrar usando nuestra herramienta "filtrar"
    tuplas_fuertes = filtrar(es_fuerte, resumen)
    
    # Extraemos solo los nombres para la respuesta final
    salas_fuertes = aplicar_a_cada(extraer_nombre, tuplas_fuertes)
    
    # 4. Extraemos los números y los duplicamos usando "aplicar_a_cada"
    totales_fuertes = aplicar_a_cada(extraer_total, tuplas_fuertes)
    proyeccion_quincenal = aplicar_a_cada(doble_valor, totales_fuertes)
    
    # 5. Calculamos la suma de las proyecciones
    gran_total_proyectado = reducir(sumar_valores, proyeccion_quincenal, 0)
