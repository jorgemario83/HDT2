...
def precio_con_descuento(precio_base, porcentaje):
    # Calculamos el monto de descuento y lo restamos al precio base
...
def precio_con_iva(precio, iva):
    # Calculamos el monto de IVA y lo sumamos al precio
    incremento = precio * (iva / 100)
...
def es_recomendada(duracion, rating):
    # Obtenemos las categorías reutilizando nuestras funciones
    categoria_duracion = clasificar_duracion(duracion)
    categoria_rating = clasificar_rating(rating)
    
    # Comprobamos las condiciones dadas con operadores lógicos
    rating_ok = (categoria_rating == "buena" or categoria_rating == "excelente")
    duracion_ok = (categoria_duracion == "estándar" or categoria_duracion == "corta")
    
    return rating_ok and duracion_ok
...
def analizar_ventas(ventas):
    # Variables iniciales para no usar sum(), len(), max(), min()
    total = 0
    contador = 0
    mejor_dia = 0
    peor_dia = 0
    
    # Asumimos que la primera venta es el máximo y mínimo temporal
    # (siempre que la lista no esté vacía)
    max_ventas = -1
    ...
    for venta in ventas:
        ...
        if venta > max_ventas:
            max_ventas = venta
            mejor_dia = indice_actual
            
        if venta < min_ventas:
            min_ventas = venta
            peor_dia = indice_actual
...
def formato_moneda(cantidad):
    # Usamos f-strings de Python que permiten formato numérico directo
    # :,  agrega separador de miles
    # .2f asegura 2 decimales
...
def barra_visual(valor, maximo, ancho=20):
    ...
    # Multiplicamos el string (símbolo) por la cantidad entera calculada
    barra = ("█" * llenos) + ("░" * vacios)
...
def linea_reporte(nombre, valor, maximo, ancho=20):
    ...
    # {nombre:<14} alinea el string a la izquierda rellenando con espacios hasta 14 caracteres
    # {moneda:<10} hace lo mismo para alinear las cantidades
    return f"{nombre:<14} | {moneda:<10} | {barra}"
